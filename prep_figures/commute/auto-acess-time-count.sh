...
#     ./access-time-count.py --tp "$($sedcmd)" $rw_path/5-taxa/123_45-1v5c/run[1-9]/uniq_trees_T -o 5-taxon-access-distn/${ntangle}.tab
# done

for ntangle in 662 665 795; do
    sedcmd="sed -n ${ntangle}p ../../tangle/rooted-symmetric/tangle6.idx"
    ./access-time-count.py --tp "$($sedcmd)" $rw_path/6-taxa/123_456-1v5c/run[1-9]/uniq_trees_T -o 6-taxon-access-distn/${ntangle}.tab
done
