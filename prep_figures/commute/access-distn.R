...
# ******* Note this filter!
d <- subset(subset(d, dist==2), t1_deg==24)
...
p <- p + geom_line()
p <- p + scale_x_continuous("access time", limits=c(2,max_index))
...
p <- p + ggtitle("5 taxa, same shape, d=2, 24 neighbors")
ggsave(paste0(as.character(n_taxa),"-taxon-access-time-kappa-short.svg"), p, width=9.5)

# Plot showing long time info.
p <- ggplot(d, aes(x=time, y=mean_dens, group=num, color=kappa)) + geom_line()
p <- p + scale_x_continuous(limits=c(1,500)) + scale_y_continuous("density", breaks=c())
