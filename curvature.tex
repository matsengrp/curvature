...
\author[Whidden and Matsen]{Christopher Whidden$^1$ and Frederick A Matsen IV$^1$}
\address{$^1$Program in Computational Biology, Fred Hutchinson Cancer Research Center, Seattle, WA, 98199}
...
As in~\cite{bordewich05,whidden2013hybridization,Whidden2015-yi}, the tree is augmented with a labelled root node $\rho$ and $\rho$ is considered a member of $X$.
We generally use $n$ to refer to the number of leaves in an $X$-tree.
For a subset $V$ of $X$, $T(V)$ is the smallest subtree of $T$ that connects all nodes in $V$. The $V$-tree induced by $T$ is the smallest tree $T|V$ that can be obtained from $T(V)$ by suppressing unlabelled nodes with fewer than two children.
...
Finally, $p_x$ is suppressed. \todo{figure}
Observe that the inclusion of $\rho$ allows for SPR moves that move subtrees to the root of the tree.

SPR operations give rise to a distance measure between $X$-trees: $\dspr{T_1, T_2}$ is the minimum number of SPR operations required to transform an $X$-tree $T_1$ into $T_2$. \todo{ref figure}
