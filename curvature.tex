...
Recall that $\curvature{T, S} := 1 - \frac{W_1(m_T, m_S)}{d(x, y)}$.
For the uniform random walk, $m_T$ is simply the set of neighbors of $T$ with each assigned a mass of $\frac{1}{\degree{T}}$.
...
Note that $\curvature[\ell_p]{T, S} := \left(1 - \frac{W_1(m_T, m_S)}{d(x, y)}\right) / p$. \todo{Do you have a good explanation for the lazy curvature definition?}
