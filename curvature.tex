...
\begin{proof}
	...
	Recall that $\curvature{T,S} = \curvature[1]{T,S} = 1 - \frac{W_1(T,S)}{\dspr{T,S}}$, and $\curvature[p]{T,S} / p = (1 - \frac{W_1'(T,S)}{\dspr{T,S}}) / p$.
	...
	For the upper bound, we observe that $$W_1'(T,S) \ge p W_1(T,S) + (1-p) \, \dspr{T,S} - \frac{2}{\max(\degree{T}, \degree{S})},$$ as at most $\frac{1}{\max(\degree{T},\degree{S})}$ of the mass can remain at each of $T$ and $S$, paired with the lazy remainder.
...
The same statement and proof of Lemma~\ref{lem:asymptotic} works with $\curvature{T,S}$ and $\ric{T,S}$ replaced by the MH random walk curvatures $\curvature{\MH; T,S}$ and $\ric{\MH; T,S}$, respectively.
We now bound the difference between the curvature of the uniform random walk $\curvature{T,S}$ and that of the MH random walk $\curvature{\MH; T,S}$.
