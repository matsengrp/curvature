...
The Wasserstein distance~\cite{Villani2003-wv} (equivalently $L^1$ transportation, or ``earth movers distance'') between $m_x$ and $m_y$ is the minimum amount of ``work'' required to move $m_x$ to $m_y$ following the graph, that is
...
Having made the connection between curvature values and access times on the SPR graphs, we now consider them theoretically.
...
\subsection{Computational investigation and validation}
We computed all of the curvatures for pairs of trees with up to seven leaves to confirm our theoretical work with direct calculation as described before.
\todo{Update once 7 taxon runs have finished}

Indeed we see that for rSPR ... (Figure~\ref{fig:rspr-scatter}). \todo{Chris: can you fill this in?}
...
We done so by developing theoretical and computational frameworks to compute Ricci-Ollivier curvature of the rSPR graph.
