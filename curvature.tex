...
%\begin{figure}
%\begin{center}
%  \includegraphics[width=4in]{figures/spr.pdf}
%\end{center}
%\caption{\
%  A rooted subtree-prune-regraft (rSPR) move~\cite{hein96}.
%}
%\label{FIGspr}
%\end{figure}
%}
...
Phylogenetic search algorithms typically explore the collection of trees by rearranging subtrees with subtree prune-and-regraft (SPR) moves (Figure~\ref{FIGspr}) or the subset of SPR moves called nearest neighbor interchanges (NNI)~\cite{robinson1971comparison}.
...
where $\Pi(m_x, m_y)$ is the set of of assignments of probability from $m_x$ to pairs of vertices in $V \times V$. \todo{FAM: cites.}
