...
Finally, we prove that the two competing notions of Ricci-Ollivier curvature (coarse and asymptotic) are closely related when applied to the rSPR graph, differing only by a small factor inversely proportional to the rSPR degrees of the given trees.
...
Ricci-Ollivier curvature provides a rigorous yet intuitive formalization of the shape of a graph with respect to a random walk (or indeed the shape of a variety of metric spaces, see~\cite{Ollivier2009-bw}).
...
\end{equation}
We use $\curvature{MH; x,y}$ to indicate curvature with respect to Metropolis-Hastings random walks (described in detail in Section~\ref{sec:random_walks}) rather than uniform random walks.
...
Lin et al.~\cite{lin2011ricci} defined a different notion of curvature in terms of lazy random walks which Loisel and Romon~\cite{Loisel2014-hu} dubbed the (asymptotic) Olivier-Ricci curvature.
The lazy random walk selects a neighbor according to the given distribution, but only travels to the neighbor with probability $p$.
We denote the coarse curvature of the lazy random walk between two trees $x$ and $y$ by $\curvature[p]{x,y}$.
For example, $\curvature[\frac{1}{4}]{x,y}$ describes the curvature of the lazy random walk that follows the given distribution with probability $\frac{1}{4}$ and remains stationary with probability $\frac{3}{4}$.
For the lazy uniform random walk, the mass assignment $m^p_x$ is $x \cup N(x)$, with each neighbor assigned mass $\frac{p}{\degree{x}}$ and $x$ assigned the remaining $1 - p$ mass.
The (asymptotic) Ricci-Ollivier curvature of $x$ and $y$ is then:
\begin{equation}
\ric{x, y} := \lim_{p \rightarrow 0} \frac{\curvature[p]{x,y}}{p}
\label{eq:curvatureDef}
\end{equation}
...
As we prove in Lemma~\ref{lem:asymptotic} the two notions of rough and asymptotic curvature differ only by a small factor bounded by $\frac{2}{\max(\degree{x}, \degree{y})}$ between adjacent trees and are equal for nonadjacent trees.
We define $\ric{MH; x, y}$ analogously to $\curvature{MH; x, y}$.
...
The analysis can be reproduced by invoking the SCons (\url{http://scons.org/}) build tool and evaluating cells in an IPython notebook; instructions are in the repository README file.
...
Finally, we proved that the two competing definitions of Ollivier-Ricci curvature, coarse and asymptotic, are closely related with respect to uniform and Metropolis-Hastings walks on the rSPR graph.
