...
Let $N(Q)$ be the set of neighbors of a tree $Q$ (this does not include $Q$).
...
\begin{proof}
	...
	Pick a neighbor $Q \in N(T) \cap N(S)$ of both $T$ and $S$ \todo{Um, why can't this intersection be empty?}.
	Then either (i) $T$ and $Q$ differ in the location of $R$, or (ii) $T$ and $Q$ differ in the location of another subtree $P$.
	In the latter case, $T|(X \setminus L(P)) = S|(X \setminus L(P))$ because $T$ and $S$ differ only in the location of $R$ and $\dspr{T,P} = \dspr{S,P} = 1$.
    \todo{FAM stopped here}
...
Let $(T'_n, S'_n)$ be any of the $o$ pairs of trees which are one SPR move apart as per Corollary~\ref{cor:paired_neighbors}.
We pair the these trees in the mass transport.
The number of trees unmatched by this pairing is $\OhOf{kn}$, and we can pair each of them arbitrarily with another tree of distance at most 3.
Thus, $W_{1,n}$ is bounded above by $(|N(T_n)| + \OhOf{kn}) / |N(T_n)|$.

A lower bound is also available because we can't do better than distance 1 for all trees except for the $\OhOf{n}$ pairs from Lemma~\ref{lem:shared_neighbors}.
...
We again pair $T'_n$ with $S'_n$ such that $\dspr{T'_n, S'_n}=1$ but, as $|N(T_n)| < |N(S_n)|$ we can only account for at most $|N(T_n)| / |N(S_n)|$ of the mass directly and may have to move the $(|N(S_n)| - |N(T_n)|) / |N(S_n)|$ remainder to trees a distance at most 3.
Thus, $W_{1,n}$ is bounded above by $(|N(T_n)| + 3\OhOf{kn}) / |N(S_n)| = (|N(S_n)| + 2\OhOf{kn}) / |N(S_n)|$.
