...
These calculations require fast new algorithms for constructing and sampling these graphs, reducing the time required to compute an rSPR graph from $O(m^2n)$-time to $O(mn^3)$, where $m$ is the (often large) number of trees in the graph and $n$ is the number of leaves in each tree, and reducing the time required to select an rSPR neighbor of a tree uniformly at random to $O(n)$ time.
...
We show using simulation that mean access time distributions depend on distance, degree, and curvature, demonstrating the relevance of these curvature results to stochastic tree search.
...
\begin{figure*}
    ...
    \caption{\
        ...
        Distribution of later rSPR $\MH$ access times for those pairs of (c) 5-taxon trees with degree 24 and (d) 6-taxon trees with degree 44.
				To limit clutter, these plots exclude pairs that are simple inclusions of pairs of trees with a smaller number of taxa (e.g. pairs of trees that both contain leaves 1 and 2 as siblings) as their access times distributions do not change substantially.
...
\begin{proof}
	...
	In the latter case, $T|(X \setminus L(Q)) = S|(X \setminus L(Q))$, where $L(Q)$ is the label set of $Q$, because $T$ and $S$ differ only in the location of $R$ and $\dspr{T,U} = \dspr{S,U} = 1$.
...
Now, we bound the average size of the moved subtrees (in terms of the number of leaves).
...
\begin{proof}
    ...
    $\size{N_i(T)} \leq 2n - 3 - 5$ for the first set of terms, and $\size{N_i(T)} > 2n - 3 - 5$ for the second, so by replacing $\size{N_i(T)}$ in each case by $2n-3-5$ we decrease the weight for $i \leq 2$ and increase the weight on $i > 2$.
