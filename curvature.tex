...
\todo[inline]{Please define x in the text of the following lemma.}
\begin{lem}
...
In our final two lemmas of this section, we consider the distribution of sizes of subtrees moved by SPR operations from a given tree.
First, we bound the number of neighbors of a tree that are induced by SPR operations involving a subtree with three or more leaves.
In particular, we show that the majority of neighbors are induced by moving small subtrees (one or two leaves).
Along with our observation from Lemma~\ref{lem:degree_change} that the magnitude of the degree change induced by an SPR operation depends on the subtree size and height change, this implies that most SPR neighbors have a similar degree.
...
\begin{proof}
	...
	Each subtree at a depth $ > 1$ from the root with $x$ nodes induces $2n - x - 5$ SPR neighbors, while the children of the root induce $2n - x - 3$ neighbors.
