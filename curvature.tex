...
Within this framework researchers typically choose either a Bayesian approach, using an algorithm such as Markov chain Monte Carlo (MCMC) to approximate the posterior distribution of trees and their associated parameters, or a maximum likelihood (ML) approach, searching across tree topologies and continuous parameters in order to find the combination with the highest likelihood.
We focus on the Bayesian approach in this paper, noting that our observations will likely also have ramifications for ML.
...
Ollivier and colleagues recently pioneered a new approach to quantifying properties of random walks~\cite{Ollivier2009-bw,Joulin2010-jg}.
They define a notion of \emph{curvature} which formalizes the notion of to what extent random walking brings points together, and is a generalization of the classical notion of Ricci curvature on manifolds.
...
However, this method is infeasible when applied to construct graphs with 7 or more leaves, due to the rapidly growing $\OhOf{m^2}$ factor.
...
Thus, we believe our fast graph construction procedure will itself be useful for further studies of rSPR graph subsets similar to~\cite{Whidden2015-yi}, as the algorithm can construct rSPR graphs for any given subset of trees.
...
We first consider properties of the uniform \cuttable{(a.k.a.\ isotropic)} random walk on the rSPR graph.
...
Recall that \cuttable[the coarse uniform random walk curvature ]{}$\curvature{T, S} := 1 - \frac{W_1(m_T, m_S)}{d(T, S)}$.
