...
\begin{figure*}
    ...
    \caption{\
        ...
        Distribution of later rSPR $\MH$ access times for those pairs of (c) 5-leaf trees with degree 24 and (d) 6-leaf trees with degree 44.
				To limit clutter, these plots exclude the relatively small number of pairs that result from modifying pairs of trees with a smaller number of leaves by replacing a given leaf with a given subtree (e.g\ we exclude pairs of trees that both contain leaves 1 and 2 as siblings). Such pairs of trees have much lower access times that are visually identical to the pairs from which they can be obtained via replacement. The net effect is that a distinct curve would appear for each smaller number of leaves. Each individual curve would show a similar influence from curvature.
...
\begin{proof}
    ...
    If not, then the $j$ leaves of the new neighbor subtree $V$ cannot be part of $k$ or $a$. We thus set $j=1$ and split the remaining $n-b-i-j = n-2$ leaves between $k$ and $a$ in as balanced a way as possible, giving (i).
	...
	If the subtree of $k$ leaves is moved to the root, then the new neighbor subtree $V$ is the entire tree excluding $R$ itself. We can thus include the root-most leaf of the tree in $a$. In this case, we gain an additional leaf over the previous case to balance the product $ka$ at the cost of increasing $j$.
...
It may be interesting future work to improve our bound by explicitly minimizing the moved subtree size in such a manner.
