...
A \emph{balanced tree} is a tree such that the sum of the depths of internal vertices is minimum over all trees with the same number of leaves.
The \emph{least common ancestor} (LCA) of a set $R$ of two or more vertices is the unique vertex that is an ancestor of each vertex $r \in R$ and at maximum depth.
Similarly, the LCA of two or more subtrees is the LCA of their parent vertices.
...
This required a fast new algorithm for SPR graph construction, reducing the time required to do so from $\OhOf{m^2 n}$, where $m$ is the number of trees in the graph ($(2n-3)!!$ in this case) to $\OhOf{mn^3}$.
...
Thus, we simply compared each of the $m$ trees to identify adjacencies, requiring a total of $\OhOf{m^2n}$-time ($\OhOf{m^2n^2}$-time in the unrooted case).
As with our curvature computations, however, an $\OhOf{m^2}$ factor where $m = (2n-3)!!$ would require an enormous amount of computation. 
...
\begin{theorem}
	The subset of the rSPR graph induced by a set $\mathcal{T}$ of $m$ trees with $n$ leaves can be constructed in $\OhOf{mn^3}$-time.
\end{theorem}
\begin{proof}
...
We then used \dots \todo{ILP, citation, github reference} to compute curvatures for given pairs of trees.
This latter step would have required an enormous amount of computation to directly compute curvatures for the $((2n-3)!!)^2$ pairs of trees with $n$ leaves.
...
We thus directly computed curvature values for one representative pair from each such equivalance class, or tanglegram. \todo{cite} \todo{indicate that a paper on tanglegrams is in preparation}
...
In contrast, the Metropolis Hastings (MH) random walk proposes a move from a tree $T$ to a neighbor tree $S$ uniformly at random and then accepts the move according to the Hastings ratio, $\min\left(1, \frac{\degree{T}}{\degree{S}}\right)$.
...
To efficiently simulate the MH random walk, we developed a linear-time algorithm for proposing SPR moves that does not require the rSPR graph to be precomputed.
Naively, this would require $\OhOf{n^3}$ time---$\OhOf{n}$ time to generate each of the $\OhOf{n^2}$ neighbors of a given tree and then pick one uniformly at random.
To eliminate an $\OhOf{n^2}$ factor, we developed a deterministic ordering of SPR moves such that an arbitrary SPR neighbor can be selected by its index in $\OhOf{n}$ time.
Song \todo{cite} developed a recursive formula for the degree of a tree and an explicit equation that works in terms of the distribution of heights of each node.
This formula does not group SPR moves that move a particularly subtree, and thus would still require $\OhOf{n^2}$ time to select a specific SPR neighbor by index.

Instead, we consider the distribution of SPR moves in terms of the number of nodes contained within a subtree.
Recall that a tree with $n$ leaves has $2n-1$ nodes.
...
In particular:
\begin{enumerate}
\item Moving $R$ to its sibling results in the same tree, not a neighboring tree,
\item Moving $R$ to its parent results in the same tree,
\item Moving $R$ to its grandparent is the same as moving its aunt to its sibling
\item Moving $R$ to its aunt is the same as moving its aunt to R.
\end{enumerate}
...
In this manner, we achieve an alternative solution for computing the degree of a tree:
\todo{equation}
