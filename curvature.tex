...
\begin{figure*}
	\hspace*{\stretch{1}}
	\subfigure[6 taxa]{\includegraphics[width=2.3in]{figs/rspr-upmh-scatter6}}
	\hspace*{\stretch{2}}
	\subfigure[7 taxa]{\includegraphics[width=2.3in]{figs/rspr-upmh-scatter7}}
	\hspace*{\stretch{1}}
...
The maximum degree difference from Lemma~\ref{lem:degree_max_delta} occurs between trees that differ by many SPR operations.
...
In our final two lemmas of this section, we consider the distribution of trees in a neighborhood with respect to the size of the subtrees moved by SPR moves.
First, we bound the number of neighbors of a tree that are induced by SPR moves involving a subtree with three or more leaves.
In particular, we show that the majority of neighbors are induced by small subtree moves (one or two leaves).
Along with our observation from Lemma~\ref{lem:degree_change} that the magnitude of the degree change induced by an SPR move depends on the subtree size and height change, this implies that most SPR neighbors have a similar degree.
	We say that a subtree is \emph{small} if it has two leaves or one leaf, and that a \emph{small} SPR move is one that moves a small subtree.

\begin{lem}
	Let $T$ be a rooted tree with $n \ge 3$ leaves.
	Let $N_S(T)$ be the neighbors of $T$ induced by small SPR moves. 
	Then $\frac{\size{N_S(T)}}{\size{N(T)}} > \frac{1}{2}$.
	\label{lem:spr_size}
\end{lem}
\begin{proof}
	Recall our distribution of rSPR moves from a tree in Lemma~\ref{lem:compute_degree}.
	Each subtree at a depth $ > 1$ from the root induces $2n - x - 5$ SPR neighbors, where $x$ is the number of nodes in the subtree, while the children of the root induce $2n - x - 3$ neighbors.
	This implies that the number of small SPR moves is directly proportional to the number of small subtrees, with a possible difference of two moves for a tree where one of the root's children is small.
	There are thus two extremes for the distribution of rSPR moves induced by small subtrees.
	A ladder tree has the smallest proportion of small subtrees.
	One 1-leaf subtree in a ladder tree is a child of the root, but this is counterbalanced by the fact that any other tree has at least one more subtree with 2 leaves.
	A balanced tree has the greatest proportion of small subtrees.
	Thus, a balanced tree has the greatest number of small SPR moves.

	We first bound the number of small subtree moves that can be applied to a ladder tree.
	...
	Therefore more than half of the neighborhood of any tree is induced by small SPR moves.
\end{proof}

Now, we show that the average size of the moved subtrees is small.

\begin{lem}
	Let $T$ be a rooted tree with $n \ge 4$ leaves.
	Let $N_i(T)$ be the neighbors of $T$ induced by SPR moves of an $i$-leaf subtree.
	Then $$\frac{\sum_i i \size{N_i(T)}}{\size{N(T)}} \le \frac{n + 2}{4} .$$
	\label{lem:spr_size_mean}
\end{lem}
\begin{proof}
	Again recall our distribution of rSPR moves from a tree in Lemma~\ref{lem:compute_degree}.
	By similar arguments as in the proof of Lemma~\ref{lem:spr_size}, a ladder tree has the greatest average moved subtree size.
	There are $n$ subtrees with 1 leaf, and 1 subtree each with $2,3, \ldots n - 1$ leaves.
	We ignore the root subtree, which does not induce any SPR moves.
	By Lemma~\ref{lem:spr_size}, at least half of the SPR moves involve a 1-leaf subtree.
	We can bound the distribution of the remaining moves with the (loose) assumption that each other subtree contributes the same number of moves.
	This bounds the mean moved subtree size from above because each successively larger subtree contributes fewer moves.
	The mean subtree size is $(2 + n - 2) / 2 = n / 2$.
	Thus, the average size of a moved subtree is at most the weighted average of $1 / 2 + (n / 2) / 2$ = $n / 4 + 1 / 2$.
...
This flatness-in-the-limit theorem thus helps explain our empirical results in Section~\ref{sec:results:curvature} showing that the distribution of curvature pairs tends more towards 0 as $n$ increases.
