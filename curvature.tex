...
In this paper, we investigate the rSPR graph in a new way: by calculating Ricci-Ollivier curvature with respect to two simple random walks.
We exactly calculate the curvature for all pairs of rooted trees up to seven leaves, finding that the curvature varies substantially across pairs of trees in the graph separated by the same distance, thus indicating significant structure to the graph beyond that seen by pairwise distance.
This calculation requires fast new algorithms for constructing and sampling these graphs, reducing the time required to compute an rSPR graph from $O(m^2n)$-time to $O(mn^3)$, where $m$ is the (often large) number of trees in the graph and $n$ their number of leaves, and reducing the time required to select an SPR neighbor of a tree uniformly at random to $O(n)$ time.
We also develop a closed form solution to characterize how the number of SPR neighbors of a tree changes after an SPR operation is applied to that tree.
This gives bounds on the curvature, as well as a flatness-in-the-limit theorem for pairs of trees differentiated by moving an asymptotically small subtree.
We also confirm using simulation that mean access time distributions depend on distance, degree, and curvature, showing the relevance of these curvature results to stochastic tree search.
\end{abstract}
...
Pairs of trees separated by such moves correspond to the ``bottlenecks" identified in our simulation results.
