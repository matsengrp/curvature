...
Let $i$ be the number of leaves in $U$, excluding any leaves of $R$, and $j$ be the number of leaves in $V$, excluding any leaves of $R$. \todo{My impression was that the leaves of $R$ and $U$ (or $V$) were disjoint. If that's the case, then this "excluding" clause is more confusing than helpful.}
...
\begin{proof}
	...
	If the subtree of $k$ leaves is moved to an ancestor, then we gain an additional leaf\todo{Hm, I'm not sure where this additional leaf is!} to balance the product but increase $j$.
...
$$\degree{S} - \degree{T} \le 2 \left(\ceilV{\frac{n-2}{2}}\floorXXL{\frac{n}{2}} +1 -  \left(\ceilV{\frac{n-2}{2}} + 1\right)\right) = 2\floorV{\frac{n-2}{2}}\ceilV{\frac{n-2}{2}}.$$
	...
	The relative change in degree, $\frac{\degree{T}}{\degree{S}}$, is minimized when $\degree{T}$ is as small as possible and $\degree{S} - \degree{T}$ is maximal.\todo{This sort of thing does not appear true in general. Please explain why it works here.}
