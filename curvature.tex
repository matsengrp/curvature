...
\todo{We have three systems of notation for the degree of trees: $\deg(T)$, this $q$ and $p$, and below $|N(T)|$. Please make this consistent, and make it clear what we actually mean by the degree of a tree.}
Then at least
$$o := q - 2kb - 2(j-1) = p - 2ka - 2(i-1)$$
...
\todo{In view of Lemma~\ref{lem:shared_neighbors} do you need the equal size statement?}
...
\todo{this belongs here if we can prove it. If, not then we should consider moving it and discuss this more in the conclusion as an area for future work.}
...
\begin{proof}
    ...
    \todo{Please make it crystal clear that these two criteria can be optimized simultaneously.}
    To bound the former effect, let $m$ be the maximum (across $T$ and $S$) proportion of mass that cannot be moved between adjacent neighbors of those trees.
...
We next bound the effect of laziness on curvature. Let $\kappa_{l,p}$\todo{conflicts with original definition of curvature $\kappa_n$. Perhaps $\kappa^{(p)}_n$?} be the curvature of a lazy random walk which moves with probability $p$ and remains stationary with probability $1 - p$.
For example, $\kappa_{l,\frac{1}{4}}$\todo{$\kappa$ does not move} moves with probability $\frac{1}{4}$ and remains stationary with probability $\frac{3}{4}$. \todo{should be defined earlier, particularly the part where the curvature is divided by $p$.}
...
We now bound the difference between the curvature of uniform random walks $\kappa$, and that of MH random walks, $\kappa_{MH}$. \todo{I suggest $\kappa^{\mathrm{MH}}_n$}.
