...
We construct a mapping from each tree $T_i$ to its order index in this ordering $i$.
...
%EM "distribution" here and below seems strange to me because we aren't always sampling. Wouldn't "enumeration" work?
...
\begin{lem}
	...
	An rSPR neighbor of a tree $T$ can be chosen uniformly at random in $\OhOf{n}$-time using $\OhOf{n}$ space (in bits).
\end{lem}
\begin{proof}
...
We used this procedure to sample random walks on varying size rSPR graphs for our analysis in the next section.
...
\begin{figure*}
	\includegraphics[width=\textwidth]{figs/access5-distribution-alabels.png}
	...
	\caption{\
		...
		See online version for color figures; data tables available at \protect\url{https://github.com/matsengrp/curvature/blob/master/prefigs/figure-5-6ab-data.zip}.
	}
...
Let $L_1$ be the $\lca{U,V}$ in $T$.
	Let $L_2$ be the $\lca{U,V}$ in $S$ (typically $L_1 = L_2$).
Let $a$ be the number of intermediate nodes on the path from $U$ to $L_2$ in $S$, excluding endpoints.
Similarly, let $b$ be the number of intermediate nodes on the path from $V$ to $L_1$ in $T$, excluding endpoints.
...
(i) subtrees that include nodes on the path from $U$ to $L_2$ may now be moved into $R$ and its newly introduced parent node,
(ii) subtrees that include nodes on the path from $V$ to $L_1$ may no longer be moved into $R$ and its parent node,
...
\end{cor}
\begin{proof}
%EM Unlike you and the reviewer, I can't figure out what source and target mean here-- are they defined?
...
For each such $(T',S')$ pair, there are two cases, depending on whether $T'$ differs from $T$ by $R$ or another subtree.
If $T'$ and $T$ differ by a different subtree, then we can move $R$ in $T'$ or $S'$ to obtain the other member of the pair.
Thus, $\dspr{T',S'} = 1$.
If $T'$ and $T$ differ by $R$, however, then $T' = S'$.
Consider the set of such pairs $(T'_1, S'_1), (T'_2, S'_2), \ldots, (T'_p, S'_p)$.
Each tree of the pairs can be obtained from the others by moving $R$, so we simply pair $T'_1$ with $S'_2$, $T'_2$ with $S'_3$ and so on, finally pairing $T'_p$ with $S'_1$.
\end{proof}
...
Recall that $k$ is the number of leaves in the moved subtree $R$, $i$ is the number of leaves in $R$'s neighbor $U$ and $j$ is the number of leaves in $R$'s new neighbor $V$ after the SPR move.
The length of the path from $U$'s root to the least common ancestor $L_2$ of $U$ and $V$ in $S$ is $a$, while $b$ is the length of the path from $V$'s root to the least common ancestor $L_1$ of $U$ and $V$ in $T$.
Call the former path $A$.

The degree change is maximized by making $L_1$ the root of T and thereby minimizing $b$, namely setting $b=0$.
...
There are two cases, depending on whether $R$ is moved to the root (thereby introducing a new level to the tree) or not.
If not, then the $j$ leaves of the new neighbor subtree $V$ cannot contribute to increasing $k$ or $a$. We thus set $j=1$ and split the remaining $n-b-i-j = n-2$ leaves between $R$ and $A$ in as balanced a way as possible, maximizing the product $ka$ and giving (i).
...
Note that these shared neighbors are exactly the trees from the second case of Corollary~\ref{cor:paired_neighbors} and an optimal mass transport pairing will pair mass at these trees.
...
\begin{proof}
    ...
    %EM Don't you mean displaces $R$ from its original position?
	In the latter case, $S$ is reached from $U$ by moving $Q$ in a manner that displaces $R$ from its original position, because $T$ and $S$ differ only in the location of $R$ and $\dspr{T,U} = \dspr{S,U} = 1$.
...
\begin{proof}
	...
	We then have that $2(2n^2 - 4n - 6) = 4n^2 - 8n - 12 \ge 4n^2 - 8n - 12 + (28 - 8n) = 4n^2 -16n + 16$, for all $n \ge 4$.
	Therefore more than half of the neighborhood of any tree is induced by small rSPR moves.
\end{proof}

Now, we bound the average size of the moved subtrees (in terms of the number of leaves) from rSPR moves according to our distribution.
Note that this average ignores some possible moves that are equivalent to moves of smaller subtrees.
This occurs because multiple NNI moves may lead to the same tree, as described previously, and each equivalent move may involve a subtree with a different number of leaves.
Our distribution includes only one of these equivalent moves.
For example, from the balanced tree with 4 leaves, we do not consider moves of the root's 2-leaf child subtrees because they are equivalent to moves of the leaves, as explained by our distribution.

\begin{lem}
	Let $T$ be a rooted tree with $n \ge 4$ leaves.
	Let $N_i(T)$ be the neighbors of $T$ induced by rSPR moves of an $i$-leaf subtree, where the moved subtree is defined according to our distribution of rSPR moves.
...
\begin{proof}
	...
	Recall from the proof of Lemma~\ref{lem:spr_size} that a ladder tree has the smallest proportion of small subtrees. This implies that a ladder tree has the greatest average moved subtree size (in terms of the number of leaves) with respect to our distribution.
	...
	Thus, the average size of a moved subtree over all SPR operations from our distribution on a given tree is at most the weighted average of $1 / 2 + ((n + 1) / 2) / 2$ = $(n + 1) / 4 + 1 / 2 = (n + 3) / 4$.
...
Further, note that our distribution does not necessarily minimize the moved subtree size among equivalent moves.
It may be interesting in future work to improve our bound by explicitly minimizing the moved subtree size in such a manner.
...
\end{thm}
\begin{proof}
	Let $W_{1,n}$ be the mass transport term with respect to $T_n$ and $S_n$, that is $W_1(m_{T_n}, m_{S_n})$. Because $d(T_n, S_n) = 1$, we will prove the theorem by showing that the mass transport term $W_{1,n}$ sits between two bounds, each of which has limit 1 as $n$ goes to infinity.
...
%EM Sorry, I don't see b and a in cor:paired_neighbors.
