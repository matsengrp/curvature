...
However, we find that large topology changes induce paths that are difficult to traverse stochastically, and which may be diagnosed by pairs of trees with negative curvature.
Our results suggest that, although these difficult to traverse regions do not impede mixing in this simple well-connected space, they may manifest as bottlenecks in the much smaller credible sets induced by phylogenetic posteriors with a likelihood function.
...
$$
We are not aware of any further work investigating properties of the SPR graph, which stems primarily from a lack of sufficient computational tools.
...
Applying the framework to Brownian motion on a manifold gives back the classical definition of Ricci curvature.
For discrete graphs, curvature is determined by the ratio of the earth mover's distance~\cite{rubner2000earth} between neighborhoods of a pair of vertices and the distance between the vertices.
...
In this paper, we investigate curvature of the rSPR graph with respect to two random walks and connect those results to access times (i.e. hitting times) for those random walks.
...
Moreover, we found that rSPR graphs with 7 or more leaves have tree pairs with negative curvature, corresponding to paths that are difficult to traverse stochastically.
This is a surprising result, as the Markov chain in this simple case is known to mix rapidly~\cite{spade2014note}.

In response, we considered properties of the graph analytically.
...
In particular, we present a full characterization of the change in rSPR degree that occurs from a given rSPR move and find that even though they each count as one move, rSPR moves which modify large subtrees may have greater graph effects than those that move small subtrees.
...
These pairs occur infrequently in these well-connected graphs, thus allowing the Markov chain to mix rapidly.
However, they may be more problematic in real posterior distributions where the majority of probability is spread over a relatively small number of trees~\cite{Whidden2015-yi}.
...
\cuttable[Figures, tables, and proofs of our theorems and lemmas can be found in the appendix.]
...
\begin{figure}
	...
	\hspace*{\stretch{2}}
	\subfigure[7 taxa]{\includegraphics[width=3in]{figs/rspr-upmh-scatter7}}
	\hspace*{\stretch{1}}
...
We applied this work to demonstrate graph effects of real MCMC posteriors sampled with MrBayes~\cite{Whidden2015-yi} using \texttt{sprspace}~\cite{sprspace}.
...
\begin{table}
\centering
\caption{p-values for ordinary least squares linear multiple regression of rSPR mean access time against degree and distance (two-tailed $t$-test of regression coefficient).}
...
\section{Curvature}
We now consider properties of the uniform \cuttable{(a.k.a.\ isotropic)} random walk on the $n$-leaf rSPR graph.
Recall that the uniform random walk begins at a tree $T$ and moves to a tree uniformly at random from $N(T)$.
\cuttable{We denote the coarse curvature of the uniform random walk between two trees $T$ and $S$ with $\curvature{T,S}$.}
Recall that \cuttable[the coarse uniform random walk curvature ]{}$\curvature{T, S} := 1 - \frac{W_1(m_T, m_S)}{d(T, S)}$, where $W_{1,n}$ is the mass transport term \eqref{eq:curvatureDef}.
For the uniform random walk, $m_T$ \cuttable[assigns]{is simply the probability measure assigning} a mass of $\frac{1}{\degree{T}}$ to each of $T$'s neighbors.
Our results follow from the lemmas of Section~\ref{sec:neighborhoods}.

\begin{restatable}{theorem}{flatcurvature}
...
Then $\lim_{n \rightarrow \infty} \curvature{T_n,S_n} = 0$\cuttable{ for the uniform random walk on the rSPR graph}.
\end{restatable}
...
Thus, our next step will be to investigate curvature of MCMC with nontrivial likelihood functions, which will \cuttable{significantly }reduce the posterior distribution to a more realistic effective size, and \cuttable{in certain cases }will lead to significant ``bottlenecks'' like those we have observed in real data.
...
Now that we have established the foundations of using curvature to understand graphs relevant for phylogenetic inference, \cuttable[many graph structures remain to be explored including NNI graphs, unrooted SPR graphs, graphs of ranked trees~\cite{Song2006-xe}, graphs of BEAST~\cite{Drummond2012-ek} rooted ``time-trees,'' and random walks on other discrete structures such as partitions~\cite{Gusfield2002-il} that can be expressed as trees.]{we will extend this work in several directions in addition to adding a nontrivial likelihood function as just described.
