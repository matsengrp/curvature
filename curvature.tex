...
\begin{proof}
	...
	Pick a neighbor $U \in N(T) \cap N(S)$ of both $T$ and $S$ \todo{Um, why can't this intersection be empty?}.
...
\todo[inline]{Do you think that we should talk about $3 \OhOf{kn}$, or just talk about $\OhOf{kn}$?}
...
\begin{lemma}
	The maximum curvature of the uniform random walk between two adjacent trees with $n$ leaves is $\frac{6n-17}{3n^2-13n+14}$. \todo{lazy random walk is $\frac{6n-15}{3n^2 - 13n + 14}$. CW: I'm a little confused here. Doesn't the curvature of the lazy random walk depend on how lazy it is?}
\end{lemma}
\begin{proof}
...
\begin{proof}
	...
	The mass transport cost is maximized (and therefore curvature minimized) by a combination of two effects: trees that must be paired at a distance $>1$ and mass that must be moved between unpaired trees due to differing degrees of $T$ and $S$\todo{It's not clear what "pairing" means when degrees differ. Thus it's confusing when it's used in the same sentence as a statement concerning differing degrees. If you want to use it as a concept, how about introducing it at the beginning of the section as a proof strategy?}
