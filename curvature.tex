...
We use random walks on the rSPR graph as being a general notion of movement around a graph, modeling phylogenetic MCMC searches.
...
The Wasserstein distance~\cite{Villani2003-wv} (equivalently $L^1$ transportation, or ``earth movers distance''~\cite{rubner2000earth}) between $m_x$ and $m_y$ is the minimum amount of ``work'' required to move $m_x$ to $m_y$ following the graph, that is
...
$\{(((1,2),3),4), ((1,2),(3,4))\}$ and
$\{(((1,4),3),4), ((1,4),(2,3))\}$
are the same after relabeling, their curvatures are the same.
...
We saw a wide variation in curvature values among tanglegrams (Figure~\ref{fig:rspr-scatter}).~\todo{EM: add 7-taxa graph when ready}
Curvature values tended to increase with increasing SPR distance.
In addition, their variance decreased with increasing distance.
Minimum curvature values for a given treespace were observed between neighboring trees, while maximum curvature values were observed between trees at maximum distance or one rSPR move closer than the maximum.
This suggests that the increased difficulty of moving between trees in a random walk due to distance may be tempered somewhat by curvature in the highly connected rSPR treespaces.

Larger treespaces tended to have smaller curvature values.
Indeed, the 7-leaf treespace contained adjacent pairs of trees with negative curvature.
Such pairs are possible bottlenecks in phylogenetic searches which may be exarcebated by likelihood or branch length constraints.
...
\begin{proof}
	...
	This implies that both $T \setminus U = S \setminus U$ and $T \setminus V = S \setminus V$ by the definition of an SPR operation.
...
\begin{proof}
	...
	Moreover, the tree $S$ can be found in $\OhOf{n}$-time by iterating over the edges of $T$ that are not contained within $u$'s subtree to select the corresponding SPR destination.
...
This required XXX and YYY time respectively.~\todo{EM: can you fill in the time this required?}
...
To calculate the empirical distributions of access times we aggregate all access times for the same tanglegram \cite{gricci, tangle}.
...
We find that the mean access time between trees $T_1$ and $T_2$ is determined by $\deg(T_1)$ and $\deg(T_2)$ (Table~\ref{tab:regressionMAT}).
Furthermore, by plotting the distribution of access times between pairs of trees with respect to their distance and curvature hints that smaller $\kappa$ slightly shifts the distribution of access times towards larger access times (Fig.~\ref{fig:short-time-kappa-access}).
...
This confirms the intuitive interpretation of $\kappa(T_1, T_2)$ as quantifying the likelihood of a random walk going from $T_1$ to $T_2$ relatively directly, certainly before the random walk achieves stationarity.
...
\begin{figure}
    ...
    \caption{\
        ...
        Color signifies rSPR distance between the trees, with green, orange, and blue signifying distances of 1, 2, and 3, respectively; the saturation of the color shows curvature $\kappa(\MH; \cdot, \cdot)$, such that increased saturation (i.e.\ darker color) indicates a smaller $\kappa$.
        }
...
Having made the connection between curvature values and access times on the SPR graphs, we now consider curvature theoretically.
...
\begin{figure}
	...
	\hspace*{\stretch{1}}
	\caption{An SPR move labelled as in Lemma~\ref{lem:degree_change}.
Moving the grey subtree $R$ from its position adjacent to $U$ in tree $T$ to its position adjacent to $V$ in tree $S$ changes the SPR degree by $2\left(k(a-b) + i - j\right)$.}
...
In summary, we have extended knowledge about the the phylogenetic rSPR graph underlying phylogenetic inference beyond previous work concerning graph diameter and vertex degree.
...
Moreover, we proved that rSPR graph degree changes depend quadratically on the product of the size of the regrafted subtree and its change in depth and that rSPR treespace tends toward flatness with respect to SPR moves that move small subtrees.

We emphasize that the influence of curvature is small in this setting of equiprobable trees (Fig.~\ref{fig:short-time-kappa-access}) and that the probability of the target node in the stationary distribution is the predominant factor determining hitting times for pairs of trees (Fig.~\ref{fig:long-time-kappa-access}).
...
In addition, we will extend this work from the rooted rSPR graph considered here to the unrooted case, which is more commonly used in phylogenetic search.
